...
function AllPosts() {
  ...
  const handleLike = async (post) => {
    ...
    try {
      const hasLiked = userLikes[post.$id]?.some(like => like.userId === userData.$id);
  ...
  const handleAddComment = async (post) => {
    ...
    try {
      ...
      setPosts(prev => prev.map(p => 
        p.$id === post.$id 
          ? { ...p, comments: updatedComments.documents } 
          : p
      ));
  ...
  const filteredPosts = posts
    ...
    .sort((a, b) => {
      if (activeTab === 'latest') {
        ...
        return (userLikes[b.$id]?.length || 0) - (userLikes[a.$id]?.length || 0);
          ...
          {filteredPosts.length === 0 ? (
            <div className="text-center py-12 bg-gray-800/30 rounded-xl border border-gray-700">
              {activeTab === 'saved' ? (
                ...
                <>
                  <p className="text-gray-400 text-lg">No posts match your search.</p>
                  <button 
                    onClick={() => setFilterQuery('')}
                    className="mt-4 inline-block px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700"
                  >
                ...
                >
                        ...
                        <div className="absolute right-0 mt-2 w-48 bg-gray-800 rounded-md shadow-lg z-20 border border-gray-700">
                          ...
                          {userData?.$id === post.UserId && (
                            <>
                              ...
                              <button
                                onClick={(e) => handleDelete(post.$id, post["Unique-image"], e)}
                    ...
                    <div className="flex gap-6">
                      <button 
                        ...
                        className={`flex items-center gap-1.5 ${
                          userLikes[post.$id]?.some(like => like.userId === userData?.$id) 
                            ? 'text-pink-500' 
                      ...
                      >
                        ...
                        />
                        <span className="text-sm font-medium">{userLikes[post.$id]?.length || 0}</span>
                      </button>
                  ...
                  <AnimatePresence>
                    {expandedPosts[post.$id] && (
                      ...
                      >
                        <div className="p-4">
                          <h3 className="text-gray-300 font-medium mb-3">Comments</h3>
                                  ...
                                  <div className="flex-1 group">
                                    <div className="bg-gray-750 p-3 rounded-lg border border-gray-700/30">
                                      <div className="flex justify-between">
                                        <p className="font-medium text-indigo-400">{comment.username}</p>
                                        <p className="text-xs text-gray-400">
